...
```bash
python scripts/mglc-live.py <端点> --yes [参数=值 ...]
```

- 免费端点（余额 / 热搜 / 低粉爆文类）无需 `--yes`，直接执行。
- 也可单独用 `python scripts/mglc-live.py --estimate <端点>` 预先查看成本。
- 金额以接口响应 `consumption` 为准，本表为参考单价。

### 第 4 步：执行并导出（已确认后）

```bash
python scripts/mglc-live.py <端点> --yes [参数=值 ...] --format excel
```
...
```bash
python scripts/mglc-live.py --list
python scripts/mglc-live.py --estimate ch-live-replays
python scripts/mglc-live.py ch-live-replays --yes --format excel url=你的链接
```
...
- 所有请求自动带 `source=mglc-live` 标识，便于用量归因与结算。
