#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""视频号直播回放台 —— 曼格云 skill（统一底座，source=mglc-live）

用法：
  python scripts/mglc-live.py --list
  python scripts/mglc-live.py --estimate <端点> [参数]
  python scripts/mglc-live.py <端点> [k=v ...] [--yes] [--format json|markdown|excel] [--report] [--output=路径] [--pages=N]
"""
import os
import sys
...
def main():
    ...
    if not positional or positional[0] in ("-h", "--help", "--list"):
        ...
        print("")
        print("示例: python scripts/mglc-live.py <端点> --yes url=链接 --format excel")
        sys.exit(EXIT_OK)
    ...
    print_estimate(est, title="视频号直播回放台 费用预估")
    if est["total"] > 0 and not yes:
        ...
        print("  python scripts/mglc-live.py {key} --yes {rest}".format(key=key, rest=rest))
        sys.exit(EXIT_OK)
