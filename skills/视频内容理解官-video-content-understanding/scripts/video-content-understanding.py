#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""视频内容理解官 —— 曼格云 skill（统一底座，source=video-content-understanding）

用法：
  python scripts/video-content-understanding.py --list
  python scripts/video-content-understanding.py --estimate <端点> [参数]
  python scripts/video-content-understanding.py <端点> [k=v ...] [--yes] [--format json|markdown|excel] [--report] [--output=路径] [--pages=N]
"""
import os
import sys
...
ENDPOINTS = ['video-understanding', 'ch-download-url']
...
def main():
    ...
    if is_help:
        ...
        print("")
        print("示例: python scripts/video-content-understanding.py <端点> --yes url=链接 --format excel")
        sys.exit(EXIT_OK)

    if is_est:
        print_estimate(estimate(positional[1:]), title="视频内容理解官 费用预估")
        sys.exit(EXIT_OK)
    ...
    print_estimate(est, title="视频内容理解官 费用预估")
    if est["total"] > 0 and not yes:
        ...
        print("  python scripts/video-content-understanding.py {key} --yes {rest}".format(key=key, rest=rest))
        sys.exit(EXIT_OK)
