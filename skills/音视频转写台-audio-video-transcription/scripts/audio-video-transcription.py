#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""音视频转写台 —— 曼格云 skill（统一底座，source=audio-video-transcription）

用法：
  python scripts/audio-video-transcription.py --list
  python scripts/audio-video-transcription.py --estimate <端点> [参数]
  python scripts/audio-video-transcription.py <端点> [k=v ...] [--yes] [--format json|markdown|excel] [--report] [--output=路径] [--pages=N]
"""
import os
import sys
...
def main():
    ...
    if is_help:
        ...
        print("")
        print("示例: python scripts/audio-video-transcription.py <端点> --yes url=链接 --format excel")
        sys.exit(EXIT_OK)
    ...
    if est["total"] > 0 and not yes:
        ...
        print("  python scripts/audio-video-transcription.py {key} --yes {rest}".format(key=key, rest=rest))
        sys.exit(EXIT_OK)
