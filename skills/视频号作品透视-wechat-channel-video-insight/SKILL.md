...
用户给一条视频号分享链接，想看这条视频的标题、账号、发布时间、封面、互动数据时使用。
...
```bash
python scripts/wechat-channel-video-insight.py <端点> --yes [参数=值 ...]
```

- 免费端点（余额 / 热搜 / 低粉爆文类）无需 `--yes`，直接执行。
- 也可单独用 `python scripts/wechat-channel-video-insight.py --estimate <端点>` 预先查看成本。
- 金额以接口响应 `consumption` 为准，本表为参考单价。

### 第 4 步：执行并导出（已确认后）

```bash
python scripts/wechat-channel-video-insight.py <端点> --yes [参数=值 ...] --format excel
```
...
```bash
python scripts/wechat-channel-video-insight.py --list
python scripts/wechat-channel-video-insight.py --estimate ch-info
python scripts/wechat-channel-video-insight.py ch-info --yes --format excel url=https://channels.weixin.qq.com/sph/xxxx
```
...
- 所有请求自动带 `source=wechat-channel-video-insight` 标识，便于用量归因与结算。
