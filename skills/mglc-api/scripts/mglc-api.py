#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""微信生态数据台 —— 曼格云 skill（统一底座，source=mglc-api）

用法：
  python scripts/mglc-api.py --list
  python scripts/mglc-api.py --estimate <端点> [参数]
  python scripts/mglc-api.py <端点> [k=v ...] [--yes] [--format json|markdown|excel] [--report] [--output=路径] [--pages=N]
"""
import os
import sys
...
def main():
    ...
    if not positional or positional[0] in ("-h", "--help", "--list"):
        ...
        print("")
        print("示例: python scripts/mglc-api.py <端点> --yes url=链接 --format excel")
        sys.exit(EXIT_OK)
    ...
    if est["total"] > 0 and not yes:
        ...
        print("  python scripts/mglc-api.py {key} --yes {rest}".format(key=key, rest=rest))
        sys.exit(EXIT_OK)
