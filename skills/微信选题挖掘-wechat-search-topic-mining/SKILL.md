...
用户要围绕某个关键词挖掘微信内的选题、看相关文章热度、拓展长尾词时使用。
...
```bash
python scripts/wechat-search-topic-mining.py <端点> --yes [参数=值 ...]
```

- 免费端点（余额 / 热搜 / 低粉爆文类）无需 `--yes`，直接执行。
- 也可单独用 `python scripts/wechat-search-topic-mining.py --estimate <端点>` 预先查看成本。
- 金额以接口响应 `consumption` 为准，本表为参考单价。

### 第 4 步：执行并导出（已确认后）

```bash
python scripts/wechat-search-topic-mining.py <端点> --yes [参数=值 ...] --format excel
```
...
```bash
python scripts/wechat-search-topic-mining.py --list
python scripts/wechat-search-topic-mining.py --estimate mp-search-summary
python scripts/wechat-search-topic-mining.py mp-search-summary --yes --format excel url=你的链接
```
...
- 所有请求自动带 `source=wechat-search-topic-mining` 标识，便于用量归因与结算。
