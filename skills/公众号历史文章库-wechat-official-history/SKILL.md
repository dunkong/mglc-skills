...
用户想看某个公众号发过哪些文章、最近更新、今日发文、按关键词翻历史时使用。
...
```bash
python scripts/wechat-official-history.py <端点> --yes [参数=值 ...]
```

- 免费端点（余额 / 热搜 / 低粉爆文类）无需 `--yes`，直接执行。
- 也可单独用 `python scripts/wechat-official-history.py --estimate <端点>` 预先查看成本。
- 金额以接口响应 `consumption` 为准，本表为参考单价。

### 第 4 步：执行并导出（已确认后）

```bash
python scripts/wechat-official-history.py <端点> --yes [参数=值 ...] --format excel
```
...
```bash
python scripts/wechat-official-history.py --list
python scripts/wechat-official-history.py --estimate mp-account-articles
python scripts/wechat-official-history.py mp-account-articles --yes --format excel url=https://mp.weixin.qq.com/s/xxxx
```
...
- 所有请求自动带 `source=wechat-official-history` 标识，便于用量归因与结算。
