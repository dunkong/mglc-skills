...
用户要按关键词/行业找公众号、了解某个公众号的资料简介时使用。
...
```bash
python scripts/wechat-official-account-finder.py <端点> --yes [参数=值 ...]
```

- 免费端点（余额 / 热搜 / 低粉爆文类）无需 `--yes`，直接执行。
- 也可单独用 `python scripts/wechat-official-account-finder.py --estimate <端点>` 预先查看成本。
- 金额以接口响应 `consumption` 为准，本表为参考单价。

### 第 4 步：执行并导出（已确认后）

```bash
python scripts/wechat-official-account-finder.py <端点> --yes [参数=值 ...] --format excel
```
...
```bash
python scripts/wechat-official-account-finder.py --list
python scripts/wechat-official-account-finder.py --estimate mp-search-accounts
python scripts/wechat-official-account-finder.py mp-search-accounts --yes --format excel query=职场
```
...
- 所有请求自动带 `source=wechat-official-account-finder` 标识，便于用量归因与结算。
