...
用户要按关键词搜索微信小程序、了解小程序 AppID 与提供方时使用。
...
```bash
python scripts/wechat-mini-program-finder.py <端点> --yes [参数=值 ...]
```

- 免费端点（余额 / 热搜 / 低粉爆文类）无需 `--yes`，直接执行。
- 也可单独用 `python scripts/wechat-mini-program-finder.py --estimate <端点>` 预先查看成本。
- 金额以接口响应 `consumption` 为准，本表为参考单价。

### 第 4 步：执行并导出（已确认后）

```bash
python scripts/wechat-mini-program-finder.py <端点> --yes [参数=值 ...] --format excel
```
...
```bash
python scripts/wechat-mini-program-finder.py --list
python scripts/wechat-mini-program-finder.py --estimate mp-search-miniprograms
python scripts/wechat-mini-program-finder.py mp-search-miniprograms --yes --format excel query=点餐
```
...
- 所有请求自动带 `source=wechat-mini-program-finder` 标识，便于用量归因与结算。
