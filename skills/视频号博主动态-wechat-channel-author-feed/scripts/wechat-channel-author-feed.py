#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""视频号博主动态 —— 曼格云 skill（统一底座，source=wechat-channel-author-feed）

用法：
  python scripts/wechat-channel-author-feed.py --list
  python scripts/wechat-channel-author-feed.py --estimate <端点> [参数]
  python scripts/wechat-channel-author-feed.py <端点> [k=v ...] [--yes] [--format json|markdown|excel] [--report] [--output=路径] [--pages=N]
"""
import os
import sys
...
ENDPOINTS = ['ch-video-list', 'ch-account-search']
...
def main():
    ...
    if is_help:
        ...
        print("")
        print("示例: python scripts/wechat-channel-author-feed.py <端点> --yes url=链接 --format excel")
        sys.exit(EXIT_OK)

    if is_est:
        print_estimate(estimate(positional[1:]), title="视频号博主动态 费用预估")
        sys.exit(EXIT_OK)
    ...
    print_estimate(est, title="视频号博主动态 费用预估")
    if est["total"] > 0 and not yes:
        ...
        print("  python scripts/wechat-channel-author-feed.py {key} --yes {rest}".format(key=key, rest=rest))
        sys.exit(EXIT_OK)
