#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""音视频转写台 —— 曼格云 skill（统一底座，source=mglc-transcribe）

用法：
  python scripts/mglc-transcribe.py --list
  python scripts/mglc-transcribe.py --estimate <端点> [参数]
  python scripts/mglc-transcribe.py <端点> [k=v ...] [--yes] [--format json|markdown|excel] [--report] [--output=路径] [--pages=N]
"""
import os
import sys
...
SLUG = "mglc-transcribe"
ENDPOINTS = ['audio-transcription', 'ch-download-url']
...
def main():
    ...
    if not positional or positional[0] in ("-h", "--help", "--list"):
        ...
        print("")
        print("示例: python scripts/mglc-transcribe.py <端点> --yes url=链接 --format excel")
        sys.exit(EXIT_OK)

    if positional[0] == "--estimate":
        print_estimate(estimate(positional[1:]), title="音视频转写台 费用预估")
        sys.exit(EXIT_OK)
    ...
    print_estimate(est, title="音视频转写台 费用预估")
    if est["total"] > 0 and not yes:
        ...
        print("  python scripts/mglc-transcribe.py {key} --yes {rest}".format(key=key, rest=rest))
        sys.exit(EXIT_OK)
